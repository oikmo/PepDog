...
public class Toolbox {
	...
	public static Vector3f QuaternionToEulerAngles(Quat4f q) {
		...
		float roll_x =(float) Math.toRadians(Math.atan2(t0, t1));

		double t2 = +2.0 * (q.w * q.y - q.z * q.x);
		t2 = (t2 > 1.0) ? (1.0 + t2) : t2;
		t2 = (t2 < -1.0) ? (-1.0 + t2) : t2;
		float pitch_y = (float)Math.toRadians(Math.asin(t2));

		double t3 = +2.0 * (q.w * q.z + q.x * q.y);
		double t4 = +1.0 - 2.0 * (q.y * q.y + q.z * q.z);
		float yaw_z = (float)Math.toRadians(Math.atan2(t3, t4));

		return new Vector3f(roll_x, pitch_y, yaw_z);
	}
	...
	public static Quat4f EulerAnglesToQuaternion(Vector3f e) {
		...
		float yaw = e.z;
		Quat4f q = new Quat4f();
		float qx = (float) Math.toDegrees(Math.sin(roll/2) * Math.cos(pitch/2) * Math.cos(yaw/2) - Math.cos(roll/2) * Math.sin(pitch/2) * Math.sin(yaw/2));
		float qy = (float) Math.toDegrees(Math.cos(roll/2) * Math.sin(pitch/2) * Math.cos(yaw/2) + Math.sin(roll/2) * Math.cos(pitch/2) * Math.sin(yaw/2));
		float qz = (float) Math.toDegrees(Math.cos(roll/2) * Math.cos(pitch/2) * Math.sin(yaw/2) - Math.sin(roll/2) * Math.sin(pitch/2) * Math.cos(yaw/2));
		float qw = (float) Math.toDegrees(Math.cos(roll/2) * Math.cos(pitch/2) * Math.cos(yaw/2) + Math.sin(roll/2) * Math.sin(pitch/2) * Math.sin(yaw/2));
		q.x = qx;
		q.y = qy;
		q.z = qz;
		q.w = qw;
